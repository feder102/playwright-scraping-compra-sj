/**
 * Scraper de otros vehículos publicados en Compra en SJ
 * Obtiene los últimos anuncios de la categoría Otros Vehículos (cat=224)
...
const OTROS_URL = `${BASE_URL}/b.php?texto=&cat=224&pagina=1&orden=10&conprecio=1&estado=Todos&foto=1&envio=0`;

async function scrapearOtros(maxAnuncios = 15) {
  console.error(`[${new Date().toISOString()}] Iniciando scrape de otros vehículos...`);

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    console.error("Cargando página de otros vehículos...");
    await page.goto(OTROS_URL, { timeout: 60000, waitUntil: 'networkidle' }).catch(() => {
      console.error("⚠️ No se pudo esperar 'networkidle', intentando con 'domcontentloaded'...");
      return page.goto(OTROS_URL, { timeout: 60000, waitUntil: 'domcontentloaded' }).catch(() => {
...
async function main() {
  try {
    const anuncios = await scrapearOtros(15);
